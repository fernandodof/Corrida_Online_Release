...
switch (filter_input(INPUT_GET, 'type')) {
    ...
    case 'currentPass': {
            ...
            $pass = EncryptPassword::encrypt(filter_input(INPUT_GET, 'password1'));
            $DBPass = $dao->getArrayResultOfNativeQueryWithParameters(Queries::GET_CURRENT_PASS, $params);
            if ($pass === $DBPass['pass']) {
                $isValid = true;
            } else {
                $isValid = false;
